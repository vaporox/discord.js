...
class GuildManager extends CachedManager {
  ...
  async fetch(options = {}) {
    ...
    if (id) {
      ...
      const data = await this.client.rest.get(Routes.guild(id), { query: { with_counts: options.withCounts ?? true } });
      return this._add(data, options.cache);
    }

    const data = await this.client.rest.get(Routes.userGuilds(), { query: options });
